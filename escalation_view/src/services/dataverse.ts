...
const TARGET_ATTACHMENT_TYPE = 931440010;
...
export async function fetchFileData(): Promise<FileData[]> {
  ...
  try {
    ...
    const filterQuery = `_proto_workorder_value eq ${currentRecordId} and proto_attachmenttype eq ${TARGET_ATTACHMENT_TYPE}`;
...
const getWorkOrderName = async (recordId: string): Promise<string> => {
  const xrm = (window.parent as any).Xrm;
  ...
  try {
    const record = await xrm.WebApi.retrieveRecord('proto_workorder', recordId, '?$select=proto_name');
...
export async function saveFileAttachment(params: SaveFileParams): Promise<FileData | null> {
  ...
  try {
    const workOrderName = await getWorkOrderName(currentRecordId);
    ...
    const attachmentQuery = `?$filter=_proto_workorder_value eq ${currentRecordId} and proto_attachmenttype eq ${TARGET_ATTACHMENT_TYPE}&$select=proto_activitymimeattachmentid`;
    ...
    if (attachmentResult.entities.length > 0) {
      ...
      const attachmentName = workOrderName ? `Attachment_${workOrderName}` : 'Attachment';
      const createResponse = await xrm.WebApi.createRecord('proto_activitymimeattachment', {
        proto_attachmenttype: TARGET_ATTACHMENT_TYPE,
        proto_attachmentname: attachmentName,
        'proto_workorder@odata.bind': `/${workOrderSetName}(${currentRecordId})`
      });
